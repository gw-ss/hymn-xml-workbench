...
test('project, file, and tab controls remain present', () => {
  for (const id of ['open-project', 'new-project', 'delete-project-launcher', 'project-save', 'project-save-message', 'project-add-input', 'project-change-input', 'project-delete-input', 'project-quit', 'major-tabs', 'source-processing-tab', 'editor-tab']) assert.equal(hasId(id), true, `missing #${id}`);
  ...
  assert.equal(hasId('unsaved-save-warning'), false);
});
...
test('resizing the Editor realigns Jianpu, lyrics, guides, and staff notation', () => {
  ...
  assert.match(styles, /\.workspace \{[^}]*width:auto;[^}]*min-width:0;[^}]*justify-self:stretch;/);
...
test('project Save writes working, revised, and alignment artifacts and clears its warning', () => {
  ...
  assert.match(app, /save\.classList\.toggle\('dirty',dirty\)/);
  assert.match(app, /message\.textContent=dirty\?'Unsaved changes\. Click here to save\.'/);
